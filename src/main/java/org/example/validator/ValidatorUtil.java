...
public class ValidatorUtil {
  ...
  public static void validateInputOfNewBook(String input) {
    ...
    if (inputContainsSingleSlash(input))  errors.add("Line should contain single dash");
    if (!inRange(author.length(), 5, 30)) errors.add("Invalid length. Name should contain more than 5 chars and less than 30 ones");
    if (invalidName(author)) errors.add("Name must contain only letters, spaces, dashes, apostrophes!");
    if (!inRange(title.length(), 5, 100)) errors.add("Invalid length. Title should contain more than 5 chars and less than 30 ones");
    if (invalidTitle(title)) errors.add("Title contains invalid symbols: |/\\\\#%=+*_><]");
  ...
  public static void validateInputOfTwoId(String input){
    ...
    if (inputContainsSingleSlash(input))  errors.add("Line should contain single dash");
    if (!bookIdErrors.isEmpty()) errors.add("\nInvalid book id:" + bookIdErrors);
    if (!readerIdErrors.isEmpty()) errors.add("\nInvalid reader id:" + readerIdErrors);
